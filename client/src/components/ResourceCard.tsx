...
export function ResourceCard({ resource, showActions = false }: ResourceCardProps) {
  ...
  const isAdmin = userRole === 'director' || userRole === 'professor';
  ...
  return (
    <div className="group bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-all duration-300 p-5 flex flex-col h-full relative overflow-hidden">
      {/* Decorative top border */}
      <div className={cn(
        "absolute top-0 left-0 w-full h-1", 
        resource.status === 'pending' ? "bg-yellow-400" : "bg-primary"
      )} />
      ...
      <div className="text-sm text-muted-foreground mb-4 space-y-1">
        <p className="font-medium text-slate-600">{resource.author || "Unknown Author"}</p>
        <p>{resource.publicationYear} • Added {format(new Date(resource.createdAt || new Date()), 'MMM d, yyyy')}</p>
      </div>
      ...
      <div className="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between gap-3">
        {resource.url && (
          ...
          >
            Access Resource <ExternalLink className="w-3 h-3 ml-2" />
          </a>
        )}
